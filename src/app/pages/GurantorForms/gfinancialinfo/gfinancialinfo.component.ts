...
@Component({
  selector: 'app-gfinancialinfo',
  templateUrl: './gfinancialinfo.component.html',
  styleUrls: ['./gfinancialinfo.component.css']
})
export class GfinancialinfoComponent implements OnInit {
  ...
  isSaved = Number(sessionStorage.getItem("G_IS_SAVED"))
  total = [];
  total1 = [];
  ...
  getSessionValues() {
    ...
    this.isSaved = Number(sessionStorage.getItem("G_IS_SAVED"))
  }

  loadInfo() {
    let filter = " AND EXTRA_INFORMATION_ID=4 AND PROPOSAL_ID=" + this.PROPOSAL_ID + " AND APPLICANT_ID=" + this.APPLICANT_ID + " AND TYPE='G'"
  ...
  loadData() {
    ...
    this.api.getFinancialInformation(this.PROPOSAL_ID, "G", this.APPLICANT_ID).subscribe(data => {
  ...
  save() {
    ...
    if (this.data.SAVING_ACCOUNT_NO != undefined && this.data.SAVING_ACCOUNT_NO.trim() != '') {
          ...
          .subscribe(successCode => {
            if (successCode['code'] == "200") {
              ...
              if (this.extraApplicantInformation.IS_APPROVED == true) {
                LOG_ACTION = 'Guarantor Financial Tab information Verified'

                DESCRIPTION = sessionStorage.getItem('userName') + ' has checked and approved the Guarantor Financial for the proposal ' + this.LOAN_KEY + ' and given the remark -' + this.extraApplicantInformation.REMARK
              } else {
                LOG_ACTION = 'Guarantor Financial Tab information Rejected'
                DESCRIPTION = sessionStorage.getItem('userName') + 'has checked and rejected the Guarantor Financial for the proposal ' + this.LOAN_KEY + ' and given the remark -' + this.extraApplicantInformation.REMARK
