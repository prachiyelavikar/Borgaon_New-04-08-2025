...
import { DepositeInBank } from '../../PersonalProposal/credit-info/Models/deposite-in-bank';
...
@Component({
  selector: 'app-ccreditinfo',
  templateUrl: './ccreditinfo.component.html',
  styleUrls: ['./ccreditinfo.component.css']
})
export class CcreditinfoComponent implements OnInit {
  ...
  data: Creditinformation = new Creditinformation();
  isSpinning = false;
  isSpinning1 = false;
  checked = false;
  indeterminate = false;
  ...
  loadInfo() {
    let filter = " AND EXTRA_INFORMATION_ID=5 AND PROPOSAL_ID=" + this.PROPOSAL_ID + " AND APPLICANT_ID=" + this.APPLICANT_ID + " AND TYPE='C'"
  ...
  getData() {
    ...
    if (this.proposaltype == "1") {
      ////console.log("I")
      filter = " AND IS_ACTIVE=1 AND TYPE='I' OR TYPE='B'"
    }
    else {
      ////console.log("F")
    ...
    this.api.getAllBankLoanTypes(0, 0, 'ID', "asc", filter)
      .subscribe(successCode => {
  ...
  save() {
    ...
    if (this.data.IS_EXISTING_LOAN_WITH_SUB == 1) {
      if (this.dataList == [] || this.dataList.length == 0) {
        this.message.error(this.bankName + this.api.translate.instant('gcreditinfo.message1'), "");
        isOk = false;
      }
    }
    ...
    if (this.data.IS_GUARANTOR_TO_LOAN_WITH_SUB == 1) {
      if (this.dataList2.length == 0) {
        this.message.error(this.api.translate.instant('gcreditinfo.label1.child1') + this.bankName + this.api.translate.instant('gcreditinfo.message3'), "");
        isOk = false;
      }
    }
    if (this.data.IS_GUARANTOR_TO_LOAN_WITH_OTHER_BANK == 1) {
      if (this.dataList7 == [] || this.dataList7.length == 0) {
        this.message.error(this.api.translate.instant('gcreditinfo.message4'), "");
        isOk = false;
      }
    }
    if (this.data.IS_EARLIAR_LOAN_HISTORY == true) {
      if (this.dataList5 == [] || this.dataList5.length == 0) {
        this.message.error(this.api.translate.instant('gcreditinfo.label5.child1') + this.bankName + this.api.translate.instant('gcreditinfo.message5'), "");
        isOk = false;
      }
    }

    if (this.data.IS_EARLIAR_LOAN_HISTORY_WITH_OTHER_BANK == 1) {
      if (this.dataList9 == [] || this.dataList9.length == 0) {
    ...
    if (this.data.IS_ANY_DEPOSITES_WITH_SUB == 1) {
      if (this.dataList4 == [] || this.dataList4.length == 0) {
        this.message.error(this.api.translate.instant('gcreditinfo.label1.child1') + this.bankName + this.api.translate.instant('gcreditinfo.message7'), "");
        isOk = false;
      }
    }
    if (this.data.IS_ANY_DEPOSITES_WITH_OTHER_BANK == 1) {
      if (this.dataList8 == [] || this.dataList8.length == 0) {
          ...
          .subscribe(successCode => {
            if (successCode['code'] == "200") {
              ...
              if (this.extraApplicantInformation.IS_APPROVED == true) {
                LOG_ACTION = 'Coborrower credit Tab information Verified'

                DESCRIPTION = sessionStorage.getItem('userName') + ' has checked and approved the Coborrower credit for the proposal ' + this.LOAN_KEY + ' and given the remark -' + this.extraApplicantInformation.REMARK
              } else {
                LOG_ACTION = 'Coborrower credit Tab information Rejected'
                DESCRIPTION = sessionStorage.getItem('userName') + 'has checked and rejected the Coborrower credit for the proposal ' + this.LOAN_KEY + ' and given the remark -' + this.extraApplicantInformation.REMARK
  ...
  add1(is_sub: boolean) {
    ...
    if (is_sub) {
      this.drawerTitle1 = this.bankName + this.api.translate.instant('gcreditinfo.drawerTitle1');
      this.drawerData1.IS_SUB = true;
      this.drawerData1.BANK_OR_INSTITUTE_NAME = this.bankName + this.api.translate.instant('gcreditinfo.name');
  ...
  delete1(data: any): void {
    ...
    this.confirmModal = this.modal.confirm({
      ...
      nzOnOk: () =>
        ...
        }).catch(() => { })//console.log('माहितीत बदल करण्यात अयशस्वी!'))
    });
  ...
  edit1(data: any): void {
    if (data.IS_SUB) {
      this.loans = this.loanData2;
      this.drawerTitle1 = this.bankName + this.api.translate.instant('gcreditinfo.drawerTitle3');
    }
    else {
  ...
  add2(is_sub2: boolean) {
    ...
    if (is_sub2) {
      this.drawerTitle2 = this.bankName + this.api.translate.instant('gcreditinfo.drawerTitle5');
      this.drawerData3 = new GuarantorForLoans();
      this.drawerData3.IS_SUB = true;
      this.drawerData3.BANK_OR_INSTITUTE_NAME = this.bankName + this.api.translate.instant('gcreditinfo.name');
  ...
  edit2(data: any): void {
    if (data.IS_SUB) {
      this.loans2 = this.loanData2;
      this.drawerTitle2 = this.bankName + this.api.translate.instant('gcreditinfo.drawerTitle7');
    }
    else {
      this.loans2 = this.loanData;
  ...
  add3(is_sub3: boolean) {
    ...
    if (is_sub3) {
      this.drawerTitle3 = this.bankName + this.api.translate.instant('gcreditinfo.drawerTitle9');
      this.drawerData5.IS_SUB = true;
      this.drawerData5.BANK_NAME = this.bankName + this.api.translate.instant('gcreditinfo.name');
    } else {
  ...
  edit3(data: any): void {
    if (data.IS_SUB)
      this.drawerTitle3 = this.bankName + this.api.translate.instant('gcreditinfo.drawerTitle11');
    else
  ...
  add4(is_sub3: boolean) {
    ...
    if (is_sub3) {
      this.drawerTitle4 = this.bankName + this.api.translate.instant('gcreditinfo.drawerTitle13');
      this.drawerData6.IS_SUB = true;
      this.drawerData6.BANK_NAME = this.bankName + this.api.translate.instant('gcreditinfo.name');
    } else {
  ...
  edit4(data: any): void {
    if (data.IS_SUB)
      this.drawerTitle4 = this.bankName + this.api.translate.instant('gcreditinfo.drawerTitle15');
    else
