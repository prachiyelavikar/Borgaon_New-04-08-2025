...
export class RentDiscoumtingLoanComponent implements OnInit {
  ...
  save(): void {
    ...
    if(this.data.IS_RENT_AGREEMENT_DONE)
    {
      ...
      if (this.data.RENT_AGREEMENT_END_DATE[0] >= 0 && this.data.RENT_AGREEMENT_END_DATE[0] 
        <= 9 && this.data.RENT_AGREEMENT_END_DATE[1] >= 0 
        && this.data.RENT_AGREEMENT_END_DATE[1] <= 9 && 
        this.data.RENT_AGREEMENT_END_DATE[3] >= 0 && this.data.RENT_AGREEMENT_END_DATE[3] <= 9 && 
        this.data.RENT_AGREEMENT_END_DATE[4] >= 0 && this.data.RENT_AGREEMENT_END_DATE[4] <= 9 && 
        this.data.RENT_AGREEMENT_END_DATE[9] >= 0 && this.data.RENT_AGREEMENT_END_DATE[9] <= 9 && 
        this.data.RENT_AGREEMENT_END_DATE[8] >= 0 && this.data.RENT_AGREEMENT_END_DATE[8] <= 9 && 
        this.data.RENT_AGREEMENT_END_DATE[7] >= 0 && this.data.RENT_AGREEMENT_END_DATE[7] <= 9 && 
        this.data.RENT_AGREEMENT_END_DATE[6] >= 0 && this.data.RENT_AGREEMENT_END_DATE[6] <= 9) {
    
        var conformedPhoneNumber = conformToMask(
          this.data.RENT_AGREEMENT_END_DATE,
          this.mask,
          { guide: false }
        )
        ...
        if (this.converted <= new Date()) {
          this.data.RENT_AGREEMENT_END_DATE = this.datePipe.transform(this.converted, 'yyyy-MM-dd');
        } 
    }
    else {
      isOk = false;
      this.message.error('Invalid Agreement end  Date','')
    }
