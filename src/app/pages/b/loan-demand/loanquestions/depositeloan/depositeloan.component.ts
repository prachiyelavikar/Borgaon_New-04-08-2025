  ...
  getdata() {
    ...
    this.api.getAlldepositLoan(0, 0, "ID", 'asc', " AND PROPOSAL_ID=" + this.PROPOSAL_ID).subscribe(data => {
      ...
      if (data['code'] == '200' && data['count'] > 0) {
        ...
        this.data.RECEIPT_DATE = this.datePipe.transform(this.data.RECEIPT_DATE, 'dd/MM/yyyy');
      }
    }, err => {
      console.log(err);
    });
       ...
       this.data.RECEIPT_DATE[7] >= 0 && this.data.RECEIPT_DATE[7] <= 9 &&
        this.data.RECEIPT_DATE[6] >= 0 && this.data.RECEIPT_DATE[6] <= 9) {
          ...
          if (this.converted <= new Date()) {
            this.data.RECEIPT_DATE = this.datePipe.transform(this.converted, 'yyyy-MM-dd');
          } else {
           
            this.message.error('Receipt Date Is Required ', '');
            isValid = false;
          }
      }
      else{
        isValid=false
        this.message.error('Invalid Receipt Date ','')
      }
