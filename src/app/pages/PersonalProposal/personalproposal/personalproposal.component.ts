...
import { RemarklistComponent } from '../remarklist/remarklist.component';
...
export class PersonalproposalComponent implements OnInit {
  ...
  subpropertyinfo: SubPropertyinfo = new SubPropertyinfo();
  addressinfoCurrent: Addressinfo = new Addressinfo();
  financial:TFinancialinfo=new TFinancialinfo()
  ...
  @ViewChild(RemarklistComponent) remark:RemarklistComponent;
  ...
  extraInfo = [];
  getLoanInfo(info: number) {
    this.isSpinningTabs = true;
    this.loanType = info;
    this.extraInfo = []
    //console.log("IndivisualInfotabs1", this.IndivisualInfotabs);
    if (this.IndivisualInfotabs.length != 0) {
      if (this.loanType == 17) {

        for (let i of this.IndivisualInfotabs) {
          if (i.EXTRA_INFORMATION_ID != 14 && i.EXTRA_INFORMATION_ID != 2) {
            this.extraInfo.push(i);
            //console.log(" this.extraInfo 1", this.extraInfo);
          }
        }
        //console.log(" this.extraInfo 2", this.extraInfo);
        this.IndivisualInfotabs = this.extraInfo;
      }
      //console.log("IndivisualInfotabs2", this.IndivisualInfotabs);
    }
  ...
  //   this.api.getAllJoints(0, 0, 'ID', "asc", "AND CREDIT_INFORMATION_ID = " + this.data.ID )
  //     .subscribe(data => {
        ...
        if (data1.PRAPOSAL_TYPE == 'वैयक्तिक') {
          ...
          this.api.getAddressInfo(proposalId, "B", applicantId).subscribe(data => {
            ...
            if (data['code'] == 200 && data['data'].length > 0) {
              ...
              this.familyDetails =  data['data'][0]['FAMILY_MEMBERS']
              var d = new Date(this.personalInfo.DATE_OF_MEMBERSHIP)
              this.personalInfo.DATE_OF_MEMBERSHIP = this.datePipe.transform(d, 'dd/MM/yyyy');
