...
export class IndustrialFinanceMachineryComponent implements OnInit {
  ...
  save(): void {
    ...
    if (this.data.MACHINERY_TYPE == 'N') {
      ...
      if (this.data.QUOTATION_GIVEN_DATE == undefined || this.data.QUOTATION_GIVEN_DATE == '') {
        isOk = false;
        this.message.error(this.api.translate.instant('industrial-finance-machinery.message14'), "");
      } else {
