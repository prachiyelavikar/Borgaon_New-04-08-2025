...
export class NewproposalComponent implements OnInit {
  ...
  save() {
...

    {
      if (this.data.PRAPOSAL_TYPE == 'वैयक्तिक') {
        ...
        if (this.data.LOAN_TYPE_ID != undefined) {
          ...
          //   this.data.TYPE_OF_AGRICULTURE_LAND = ''
          //   this.data.CURRENT_AGRICULTURE_PRODUCT = ''
          //   this.data.ANNUAL_INCOME_FROM_THIS_LAND = null
          //   this.data['AGRI_ADDRESS'] = ''
          //   this.data.VILLAGE_NAME = ''
          //   // if (
          //   //   this.addressinfoCurrent2.DISTRICT != undefined && this.addressinfoCurrent2.DISTRICT != "" &&
          //   //   this.addressinfoCurrent2.HOUSE_NO != undefined && this.addressinfoCurrent2.HOUSE_NO != "" &&
          //   //   this.addressinfoCurrent2.PINCODE != undefined && this.addressinfoCurrent2.PINCODE != "" &&
          //   //   this.addressinfoCurrent2.TALUKA != undefined && this.addressinfoCurrent2.TALUKA != "" &&
          //   //   this.addressinfoCurrent2.STATE != undefined && this.addressinfoCurrent2.STATE != "" &&
          //   //   this.addressinfoCurrent2.VILLAGE != undefined && this.addressinfoCurrent2.VILLAGE != ""
          //   // ) {
          //   //   if (!this.isValidPincode(this.addressinfoCurrent2.PINCODE)) {
          //   //     Isok = false;
          //   //     this.message.error(this.api.translate.instant('business_info.business_info_pincode1_nzErrorTip'), "");
          //   //   } else {
          //   //     this.data['AGRI_ADDRESS'] = this.addressinfoCurrent2
          //   //   }

          //   //   if (this.data.VILLAGE_NAME == undefined || this.data.VILLAGE_NAME.trim() == "") {
          //   //     Isok = false;
          //   //   }

          //   //   if (this.data.TOTAL_AREA_IN_APPLICANT_NAME == undefined || this.data.TOTAL_AREA_IN_APPLICANT_NAME.toString().trim() == "") {
          //   //     Isok = false;
          //   //     this.message.error(this.api.translate.instant('agri_info.message2'), "");
          //   //   }
          //   //   if (this.data['bagayat'] && this.data['jirayat']) {
          //   //     this.data.TYPE_OF_AGRICULTURE_LAND = "B,G";
          //   //   }

          //   //   if (this.data['bagayat'] && !this.data['jirayat']) {
          //   //     this.data.TYPE_OF_AGRICULTURE_LAND = "B";
          //   //   }
          //   //   if (!this.data['bagayat'] && this.data['jirayat']) {
          //   //     this.data.TYPE_OF_AGRICULTURE_LAND = "G";
          //   //   }
    ...
    if (Isok) {
      ...
      this.data.BRANCH_ID = Number(sessionStorage.getItem('branchId'))
      this.data.ASSING_BRANCH_ID = Number(sessionStorage.getItem('branchId'))



      for (let i = 0; i < this.branchData.length; i++) {
        if (this.branchData[i].ID == this.data.BRANCH_ID) {
          this.data.BRANCH_NAME = this.branchData[i].NAME;
          this.data.BRANCH_NAME_EN = this.branchData[i].NAME_EN;
          this.data.BRANCH_NAME_KN = this.branchData[i].NAME_KN;
  ...
  changecustomer() {
    this.api.searchCustomer(this.searchText).subscribe({
      next: (res) => {
        if (res['code'] == 200) {
          console.log(res['data']);
          let customer_data = res['data'][0];
          this.data.APPLICANT_NAME = customer_data.NAME;
          this.data.PAN = customer_data.PAN;
          this.data.MOBILE_NO1 = customer_data.MOBILE_NO;
          this.data.DOB = customer_data.DOB;
          this.data.AADHAR = customer_data.AADHAAR;
          this.data.EMAIL_ID = customer_data.EMAIL_ID;
          this.data.GENDER = customer_data.GENDER;
          this.data.EDUCATION = customer_data.EDUCATION;
          this.data.MEMBERSHIP_AMOUNT = customer_data.MEMBERSHIP_AMOUNT;
          this.data.MEMBERSHIP_NUMBER = customer_data.MEMBERSHIP_NUMBER;
          this.data.OCCUPATION = customer_data.OCCUPATION;
          this.data.MEMBERS_IN_FAMILY = customer_data.MEMBERS_IN_FAMILY;

          this.onChange(this.data.DOB);
          this.data.CURRENT_ADDRESS_ID = customer_data.CURRENT_ADDRESS_ID;



          this.message.success("User Found.", '');

          // ✅ Address मिळवण्यासाठी API कॉल
          // if (customer_data.ID) {  
          //   this.getCustomerAddress(customer_data.ID); 
          // }


          // if (customer_data.CURRENT_ADDRESS_ID) {
          //   this.api.getAllAddressInformation(0, 0, "ID", "asc", "AND ID =" + customer_data.CURRENT_ADDRESS_ID).subscribe(data => {
          //     if (data['code'] == 200) {
          //       let addressData = data['data'][0];

          //       // console.log('Address Data:', addressData);
          //       // this.addressinfoCurrent = addressData['data'][0];

          //       // this.addressinfoCurrent = addressData['CURRENT_ADDRESS']; 

          //       if (addressData['CURRENT_ADDRESS'] && addressData['CURRENT_ADDRESS'].length > 0) {
          //         this.addressinfoCurrent = addressData['data'][0];
          ...
          if (customer_data.CURRENT_ADDRESS_ID) {
            this.api.getAllAddressInformation(0, 0, "ID", "asc", "AND ID =" + customer_data.CURRENT_ADDRESS_ID).subscribe({
              // next: (data) => {
              //   if (data['code'] == 200) {
              //       let addressData = data['data'][0];
              //       this.addressinfoCurrent = { ...addressData['CURRENT_ADDRESS'][0] }; // Correct way to copy object
              //       this.data['CURRENT_ADDRESS'] = addressData['CURRENT_ADDRESS'];
              //   } else {
              //     this.message.error("Address Not Found.", '');
              //   }
              // },
              ...
              next: (addressdata) => {
                if (addressdata['code'] == 200) {
                  this.addressinfoCurrent = addressdata['data'][0]; // Assign address object
                  this.data['CURRENT_ADDRESS'] = [this.addressinfoCurrent]; // Store in CURRENT_ADDRESS
                } else {
                  this.message.error("Address Not visible.", '');
                }
              },
              error: (err) => {
                console.error('Error fetching address:', err);
                this.message.error("Wrong Address.", '');
              }
            });
          }
          ...
          this.message.error("Customer Not Found.", '');
        }
        else {
  ...
  changepan(PAN_NO) {
    if (PAN_NO.length == 10) {
      ...
      this.api.getPan(PAN_NO).
        subscribe(data1 => {
          if (data1['code'] == 200 && data1['count'] > 0) {
            this.data.PAN_GRADING = data1['data'][0]['GRADE']
          }
          else {
