...
@Component({
  selector: 'app-disbursed-proposals-list',
  templateUrl: './disbursed-proposals-list.component.html',
  styleUrls: ['./disbursed-proposals-list.component.css'],
  providers: [DatePipe]
})
export class DisbursedProposalsListComponent implements OnInit {
  ...
  search(reset: boolean = false) {
    ...
    var filter = "AND CURRENT_STAGE_ID = 13 AND LOAN_OFFICER_ID = " + this.userId

    // if (likeQuery && !this.filterQuery)
    //   filter = filter + likeQuery
    // else if (this.filterQuery && !likeQuery) {
    //   filter = "AND LOAN_AMOUNT <= " + sessionStorage.getItem('loanlimit') + this.filterQuery;
    // }
    // else if (likeQuery && this.filterQuery) {
    //   filter = "AND LOAN_AMOUNT <= " + sessionStorage.getItem('loanlimit') + this.filterQuery + likeQuery;
    ...
    // var filter = "AND (CURRENT_STAGE_ID = 14 OR CURRENT_STAGE_ID = 12 OR CURRENT_STAGE_ID = 6 OR CURRENT_STAGE_ID = 13)  AND LOAN_OFFICER_ID = " + this.userId
    ...
    // if(this.LOAN_OFFICER_ID == '17'){

    //   let filter = " AND CURRENT_STAGE_ID = 13 AND LOAN_AMOUNT <= " + sessionStorage.getItem('loanlimit') + " AND BRANCH_ID IN (" + ((this.assignho1.toString())) + ")"
    //   this.api.getAllPraposals(this.pageIndex, this.pageSize, this.sortKey, sort, filter).subscribe(data => {
    //     if (data['code'] == 200) {
    //       this.loadingRecords = false;
    //       this.isSpinning = false
