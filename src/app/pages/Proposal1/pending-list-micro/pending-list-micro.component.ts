...
@Component({
  selector: 'app-pending-list-micro',
  templateUrl: './pending-list-micro.component.html',
  styleUrls: ['./pending-list-micro.component.css']
})
export class PendingListMicroComponent implements OnInit {
  loadingRecords = true;
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  ...
  ngOnInit(): void {
    ...
    this.api.reconsideredProposals$.subscribe((reconsideredProposals) => {
      this.applyReconsideredStyles(reconsideredProposals);
    });
  ...
  applyReconsideredStyles(reconsideredProposals: number[]) {
    this.dataList.forEach((proposal) => {
      // Check if the proposal ID is in the reconsideredProposals array
      proposal.IS_RECONSIDERED = reconsideredProposals.includes(proposal.ID);
    });
  }

  getData(){
    var sort: string;
    var filter = "AND CURRENT_STAGE_ID = 16"

    this.data.IS_RECONSIDERED = true
