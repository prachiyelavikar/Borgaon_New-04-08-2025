...
export class LoanoffComponent implements OnInit {
  ...
  //   this.api.getUserFilterQuery(Number(sessionStorage.getItem('userId'))).subscribe({
  //     next:(res)=>{
  ...
  //         if(this.roleId =='12'){
  //           filter += ` AND ASSISTANT_LO_ID IS NULL AND CURRENT_STAGE_ID = 19`
  //         }
  //         else if(this.roleId =='14'){
  //           filter += ` AND LOAN_OFFICER_ID IS NULL AND CURRENT_STAGE_ID = 14`
  //         }
  ...
  //         this.message.info("Please confgure the user",'')
  //       }
  //       else{
  ...
  //     },
  //     error:(error)=>{
  //       console.log("error get proposal for loanofficer",error);
  //       this.message.error("Something Went wrong","")
  //     }
  //   });
  ...
  //   // var filter =" AND (CURRENT_STAGE_ID = 14 OR CURRENT_STAGE_ID = 12 OR CURRENT_STAGE_ID = 6 OR CURRENT_STAGE_ID = 13) AND LOAN_OFFICER_ID = 0 AND LOAN_AMOUNT <= " + sessionStorage.getItem('loanlimit') +  " AND BRANCH_ID IN (" + ((this.assignho1.toString())) +")" + likeQuery
  ...
  search(reset: boolean = false) {
    ...
    try {
        ...
        this.logtext = "Filter Applied - Proposals form " + sort + " " + this.sortKey + " KEYWORD [F - Proposals] ";
        this.api.addLog('A', this.logtext, this.api.emailId)
            .subscribe(successCode => {
                if (successCode['code'] == "200") {
                    // console.log(successCode);
    ...
    // Ensure only CURRENT_STAGE_ID = 7 OR 18 proposals are shown
    // filter += " AND (CURRENT_STAGE_ID = 19)";
    ...
    this.api.getAllPraposals(this.pageIndex, this.pageSize, this.sortKey, sort, filter)
        .subscribe(data => {
            this.loadingRecords = false;
            this.isSpinning = false;
            this.totalRecords = data['count'];
    ...
    this.api.getAllPraposals(0, 0, this.sortKey, sort, filter)
        .subscribe(data => {
            this.dataList1 = data['data'];
        }, err => {
            if (err['ok'] == false) {
  ...
  updateloan(data:Proposal){
    ...
    if(this.roleId =='12'){
      data.ASSISTANT_LO_ID = Number(this.userId)
    }
    else if(this.roleId =='14'){
      data.LOAN_OFFICER_ID = Number(this.userId)
    }

    this.api.updateProposal1(data).subscribe({
      next:(res)=>{
        if(res['code'] == 200){
          this.search();
        }
        else{
          this.message.error("Failed to pick this proposal","");
        }
      },
      error:(error)=>{
        console.log("Error in pick proposal",error);
        this.message.error("Failed to pick this proposal","");
      }
    })
