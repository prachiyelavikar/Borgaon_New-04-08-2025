...
export class ApprovedListComponent implements OnInit {
  ...
  getdata() {
    ...
    this.api.getAllAssignBranches1(0, 0, '', '', filter).subscribe(data => {
      ...
      for (let i = 0; i < this.dataassBranch.length; i++) {
        ...
        console.log("asignbranchessssssssssssssssssss", this.assignho1)
      }
    }, err => {
      //console.log(err);
    });
  ...
  search(reset: boolean = false) {
    ...
    if (sessionStorage.getItem('roleId') == '7') {
      var filter = " AND (CURRENT_STAGE_ID = 12) AND LOAN_AMOUNT <= " + sessionStorage.getItem('loanlimit') + " AND BRANCH_ID IN (" + ((this.assignho1.toString())) + ")"
    } else {
      var filter = " AND (CURRENT_STAGE_ID = 12) AND LOAN_AMOUNT >= " + sessionStorage.getItem('loanlimit')
