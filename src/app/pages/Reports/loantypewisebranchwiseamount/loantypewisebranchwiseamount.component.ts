...
@Component({
  selector: 'app-loantypewisebranchwiseamount',
  templateUrl: './loantypewisebranchwiseamount.component.html',
  styleUrls: ['./loantypewisebranchwiseamount.component.css'],
  providers: [DatePipe]
})
export class LoantypewisebranchwiseamountComponent implements OnInit {
  ...
  exportbutton = false;
  columns: string[][] = [['NAME_MR', this.api.translate.instant('loantypes.columns1')], ['TOTAL', 'Total']]
  usertype = 'A';
  ...
  ngOnInit() {
    ...
    this.search();
    this.logtext = "OPENED - LoanType Wise Stage Wise Amount form KEYWORD[O - Enquiries] ";
    ...
    this.userActivityLogData.ACTIVITY_DETAILS = "LoanType Wise Stage Wise Amount  - Opened"
  ...
  search(reset: boolean = false) {
    ...
    try {
      ...
      this.logtext = "Filter Applied - LoanType Wise Stage Wise Amount  form" + sort + " " + this.sortKey + " KEYWORD [F - Praposals] ";
      this.api.addLog('A', this.logtext, this.api.emailId)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
      ...
      this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
      this.userActivityLogData.ACTIVITY_DETAILS = "LoanTypeWiseStageWiseAmount  - Sort on " + sort + " " + this.sortKey
      this.userActivityLogData.ACTIVITY_TIME = new Date()
      this.api.createUserActivityLog(this.userActivityLogData)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
    ...
    this.logtext = "Filter Applied - LoanType Wise Stage Wise Amount  form " + likeQuery + " KEYWORD [F - Praposals] ";
    this.api.addLog('A', this.logtext, this.api.emailId)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
    this.userActivityLogData.ACTIVITY_DETAILS = "LoanTypeWiseStageWiseAmount  - Search For " + this.searchText + " " + likeQuery
    this.userActivityLogData.ACTIVITY_TIME = new Date()
    this.api.createUserActivityLog(this.userActivityLogData)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.dataList = []
    this.api.getBranchLoanTypeStageProposalcountReport(this.value1, this.value2, this.LOAN_TYPE_ID).subscribe(data => {
