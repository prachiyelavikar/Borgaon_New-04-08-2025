...
@Component({
  selector: 'app-industrialmarkingloan',
  templateUrl: './industrialmarkingloan.component.html',
  styleUrls: ['./industrialmarkingloan.component.css']
})
export class IndustrialmarkingloanComponent implements OnInit {
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList = [];
  ...
  ngOnInit(): void {
    ...
    this.loadLoanTypes()
    this.logtext = "OPENED - Industrial marking Loan  KEYWORD[O - Praposals] ";
    ...
    this.userActivityLogData.ACTIVITY_DETAILS = "IndustrialmarkingLoan  - Opened"
  ...
  search(reset: boolean = false) {
    ...
    try {
      ...
      this.logtext = "Filter Applied - Industrial marking Loan " + sort + " " + this.sortKey + " KEYWORD [F - Praposals] ";
      this.api.addLog('A', this.logtext, this.api.emailId)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
            ////console.log(successCode);
      ...
      this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
      this.userActivityLogData.ACTIVITY_DETAILS = "IndustrialmarkingLoan  - Sort on " + sort + " " + this.sortKey
      this.userActivityLogData.ACTIVITY_TIME = new Date()
      this.api.createUserActivityLog(this.userActivityLogData)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
    ...
    this.logtext = "Filter Applied - Industrial marking Loan  " + likeQuery + " KEYWORD [F - Praposals] ";
    this.api.addLog('A', this.logtext, this.api.emailId)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
          ////console.log(successCode);
    ...
    this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
    this.userActivityLogData.ACTIVITY_DETAILS = "IndustrialmarkingLoan  - Search For " + this.searchText + " " + likeQuery
    this.userActivityLogData.ACTIVITY_TIME = new Date()
    this.api.createUserActivityLog(this.userActivityLogData)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.api.getIndustrialmarkingLoanReport(this.pageIndex, this.pageSize, this.sortKey, sort, filter).subscribe(data => {
