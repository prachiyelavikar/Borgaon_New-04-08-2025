...
@Component({
  selector: 'app-applicantsummery',
  templateUrl: './applicantsummery.component.html',
  styleUrls: ['./applicantsummery.component.css']
})
export class ApplicantsummeryComponent implements OnInit {
  ...
  columns: string[][] = [
    ['NAME', this.api.translate.instant('users.coloumn1')],
    ['MOBILE_NO', this.api.translate.instant('users.coloumn3')],
    ['PROPOSAL_COUNT', 'Proposals Count'],
    ['DOCUMENT_MAPPED_COUNT', 'Document Mapped Count '],
    ['DOCUMENT_UPLOADED_COUNT', 'Document uploaded Count'],
    ['APPROVED_PROPOSALS', 'Approved Proposals'],
    ['REJECTED_PROPOSALS', 'Rejected Proposals'],
    ['ONHOLD_PROPOSALS', 'Onhold Proposals'],
  ]
  usertype = 'A';
  ...
  ngOnInit() {
    ...
    // }
    this.search();
    this.logtext = "OPENED - Applicant Summary form KEYWORD[O - Enquiries] ";
    ...
    this.userActivityLogData.ACTIVITY_DETAILS = "Applicant Summary  - Opened"
  ...
  search(reset: boolean = false) {
    ...
    try {
      ...
      this.logtext = "Filter Applied - Applicant Summary  form" + sort + " " + this.sortKey + " KEYWORD [F - Praposals] ";
      this.api.addLog('A', this.logtext, this.api.emailId)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
      ...
      this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
      this.userActivityLogData.ACTIVITY_DETAILS = "ApplicantSummary  - Sort on " + sort + " " + this.sortKey
      this.userActivityLogData.ACTIVITY_TIME = new Date()
      this.api.createUserActivityLog(this.userActivityLogData)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
    ...
    this.logtext = "Filter Applied - Applicant Summary  form " + likeQuery + " KEYWORD [F - Praposals] ";
    this.api.addLog('A', this.logtext, this.api.emailId)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
    this.userActivityLogData.ACTIVITY_DETAILS = "ApplicantSummary  - Search For " + this.searchText + " " + likeQuery
    this.userActivityLogData.ACTIVITY_TIME = new Date()
    this.api.createUserActivityLog(this.userActivityLogData)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.dataList = []
    this.api.getApplicantMaster(this.pageIndex, this.pageSize, this.sortKey, sort, filter).subscribe(data => {
