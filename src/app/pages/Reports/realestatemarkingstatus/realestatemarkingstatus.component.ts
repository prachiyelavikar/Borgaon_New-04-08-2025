...
@Component({
  selector: 'app-realestatemarkingstatus',
  templateUrl: './realestatemarkingstatus.component.html',
  styleUrls: ['./realestatemarkingstatus.component.css']
})
export class RealestatemarkingstatusComponent implements OnInit {
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList = [];
  ...
  ngOnInit(): void {
    ...
    // this.loadLoanTypes()
    this.logtext = "OPENED - Real Estate Marking Status  KEYWORD[O - Praposals] ";
    ...
    this.userActivityLogData.ACTIVITY_DETAILS = "RealEstateMarkingstatus  - Opened"
  ...
  search(reset: boolean = false) {
    ...
    try {
      ...
      this.logtext = "Filter Applied - Real Estate Marking Status " + sort + " " + this.sortKey + " KEYWORD [F - Praposals] ";
      this.api.addLog('A', this.logtext, this.api.emailId)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
            ////console.log(successCode);
      ...
      this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
      this.userActivityLogData.ACTIVITY_DETAILS = "RealEstateMarkingstatus  - Sort on " + sort + " " + this.sortKey
      this.userActivityLogData.ACTIVITY_TIME = new Date()
      this.api.createUserActivityLog(this.userActivityLogData)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
    ...
    this.logtext = "Filter Applied - Real Estate Marking Status  " + likeQuery + " KEYWORD [F - Praposals] ";
    this.api.addLog('A', this.logtext, this.api.emailId)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
          ////console.log(successCode);
    ...
    this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
    this.userActivityLogData.ACTIVITY_DETAILS = "RealEstateMarkingstatus  - Search For " + this.searchText + " " + likeQuery
    this.userActivityLogData.ACTIVITY_TIME = new Date()
    this.api.createUserActivityLog(this.userActivityLogData)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.api.getRealEstateMarkingStatus(this.pageIndex, this.pageSize, this.sortKey, sort, filter).subscribe(data => {
