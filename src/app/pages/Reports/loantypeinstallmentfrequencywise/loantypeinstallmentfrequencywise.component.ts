...
@Component({
  selector: 'app-loantypeinstallmentfrequencywise',
  templateUrl: './loantypeinstallmentfrequencywise.component.html',
  styleUrls: ['./loantypeinstallmentfrequencywise.component.css'],
  providers: [DatePipe]
})
export class LoantypeinstallmentfrequencywiseComponent implements OnInit {
  ...
  exportbutton = false;
  columns: string[][] = [['NAME_MR', this.api.translate.instant('branch.label2.Name')]]
  usertype = 'A';
  ...
  ngOnInit() {
    ...
    this.logtext = "OPENED - Loan Type wise installment frequency wise count form KEYWORD[O - Enquiries] ";
    ...
    this.userActivityLogData.ACTIVITY_DETAILS = "Loan Type wise installment frequency wise count  - Opened"
  ...
  search(reset: boolean = false) {
    ...
    try {
      ...
      this.logtext = "Filter Applied - Loan Type wise installment frequency wise count  form" + sort + " " + this.sortKey + " KEYWORD [F - Praposals] ";
      this.api.addLog('A', this.logtext, this.api.emailId)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
      ...
      this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
      this.userActivityLogData.ACTIVITY_DETAILS = "LoanTypewiseinstallmentfrequencywise   - Sort on " + sort + " " + this.sortKey
      this.userActivityLogData.ACTIVITY_TIME = new Date()
      this.api.createUserActivityLog(this.userActivityLogData)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
    ...
    this.logtext = "Filter Applied - Loan Type wise installment frequency wise count  form " + likeQuery + " KEYWORD [F - Praposals] ";
    this.api.addLog('A', this.logtext, this.api.emailId)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
    this.userActivityLogData.ACTIVITY_DETAILS = "LoanTypewiseinstallmentfrequencywise   - Search For " + this.searchText + " " + likeQuery
    this.userActivityLogData.ACTIVITY_TIME = new Date()
    this.api.createUserActivityLog(this.userActivityLogData)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.dataList = []
    this.api.getLoanTypeInstallmentFrequencyWisePraposalCount(this.PROPOSAL_STAGE_ID).subscribe(data => {
