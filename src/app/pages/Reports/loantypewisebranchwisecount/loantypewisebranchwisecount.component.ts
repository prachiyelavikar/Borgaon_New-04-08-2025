...
@Component({
  selector: 'app-loantypewisebranchwisecount',
  templateUrl: './loantypewisebranchwisecount.component.html',
  styleUrls: ['./loantypewisebranchwisecount.component.css'],
  providers: [DatePipe]
})
export class LoantypewisebranchwisecountComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.search();
    this.logtext = "OPENED - LoanType Wise Stage Wise Count form KEYWORD[O - Enquiries] ";
    ...
    this.userActivityLogData.ACTIVITY_DETAILS = "LoanType Wise Stage Wise Count  - Opened"
  ...
  search(reset: boolean = false) {
    ...
    try {
      ...
      this.logtext = "Filter Applied - LoanType Wise Stage Wise Count  form" + sort + " " + this.sortKey + " KEYWORD [F - Praposals] ";
      this.api.addLog('A', this.logtext, this.api.emailId)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
      ...
      this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
      this.userActivityLogData.ACTIVITY_DETAILS = "LoanTypeWiseStageWiseCount  - Sort on " + sort + " " + this.sortKey
      this.userActivityLogData.ACTIVITY_TIME = new Date()
      this.api.createUserActivityLog(this.userActivityLogData)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
    ...
    this.logtext = "Filter Applied - LoanType Wise Stage Wise Count  form " + likeQuery + " KEYWORD [F - Praposals] ";
    this.api.addLog('A', this.logtext, this.api.emailId)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
    this.userActivityLogData.ACTIVITY_DETAILS = "LoanTypeWiseStageWiseCount  - Search For " + this.searchText + " " + likeQuery
    this.userActivityLogData.ACTIVITY_TIME = new Date()
    this.api.createUserActivityLog(this.userActivityLogData)
      .subscribe(successCode => {
        if (successCode['code'] == "200") {
    ...
    this.dataList = []
    this.api.getBranchLoanTypeStageProposalAmountReport(this.value1, this.value2, this.LOAN_TYPE_ID).subscribe(data => {
