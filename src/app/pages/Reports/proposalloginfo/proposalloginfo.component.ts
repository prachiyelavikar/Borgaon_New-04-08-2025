...
@Component({
  selector: 'app-proposalloginfo',
  templateUrl: './proposalloginfo.component.html',
  styleUrls: ['./proposalloginfo.component.css'],
  providers: [DatePipe]
})
export class ProposalloginfoComponent implements OnInit {
  formTitle = "Proposal Log Information";
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList = [];
  ...
  ngOnInit() {
    ...
    if (this.browserLang == 'mr') {
      // , acivity datetime
      this.columns = [['USER_NAME', 'User Name'],['LOAN_KEY', this.api.translate.instant('proposalsall.columns1')],['DESCRIPTION', 'Activity Details'],['LOG_DATETIME', 'Activity DateTime']]
    ...
    this.logtext = "OPENED - Proposal Log Information form KEYWORD[O - Proposal Log Information] ";
    ...
    this.userActivityLogData.ACTIVITY_DETAILS = "Proposal Log Information - Opened"
  ...
  search(reset: boolean = false) {
    ...
    try {
      ...
      this.logtext = "Filter Applied - Proposal Log Information form" + sort + " " + this.sortKey + " KEYWORD [F - Proposal Log Information] ";
      this.api.addLog('A', this.logtext, this.api.emailId)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
            //console.log(successCode);
      ...
      this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
      this.userActivityLogData.ACTIVITY_DETAILS = "Proposal Log Information - Sort on " + sort + " " + this.sortKey
      this.userActivityLogData.ACTIVITY_TIME = new Date()
      this.api.createUserActivityLog(this.userActivityLogData)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
    ...
    if (this.searchText != "") {
      ...
      this.logtext = "Filter Applied - Proposal Log Information form " + likeQuery + " KEYWORD [F - Proposal Log Information] ";
      this.api.addLog('A', this.logtext, this.api.emailId)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
            //console.log(successCode);
      ...
      this.userActivityLogData.USER_ID = Number(sessionStorage.getItem('userId'))
      this.userActivityLogData.ACTIVITY_DETAILS = "Proposal Log Information - Search For " + this.searchText + " " + likeQuery
      this.userActivityLogData.ACTIVITY_TIME = new Date()
      this.api.createUserActivityLog(this.userActivityLogData)
        .subscribe(successCode => {
          if (successCode['code'] == "200") {
